...
const OrchestratorRouting = ({ routing }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 p-3 rounded-lg bg-zinc-900/60 border border-zinc-800/40">
      ...
      {routing.urgency && (
        <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full"
          style={{ color: urgencyColor[routing.urgency] || "#94a3b8",
                   backgroundColor: `${urgencyColor[routing.urgency] || "#94a3b8"}15` }}>
          {routing.urgency}
        </span>
      )}
      {routing.reasoning && (
        <p className="w-full text-xs text-zinc-500 mt-1">{routing.reasoning}</p>
      )}
    </div>
  );
};
